...
$(document).on('change', '#anio_vehiculo_buscar', function(){
  ...
  var list ='<option value="0">Select Tamanio Motor</option>';
  $("#tam_motor_buscar").empty();
...
$(document).on('change', '#clasificacion_pieza_buscar', function(){
  ...
  var auto_modelo = $("#tipo_vehiculo_buscar").val();
  var marca = $("#marca_vehiculo_buscar").val();
  var motor = $("#tam_motor_buscar").val();
...
$(document).on('click', '#buscar_', function(e){
...
function buscarParte(){
  ...
  var motor = $("#tam_motor_buscar").val(); //Text
  ...
  var num_motor ="null";
  var chasis = "null";
  if( $("#num_motor_buscar").val())
  {
    num_motor =$("#num_motor_buscar").val();
  }
  if( $("#num_chasis_buscar").val());
  {
    chasis = $("#num_chasis_buscar").val();
  }


  $.post('models/requests/piezasVehiculo.php', { type: 'get_piezasVehiculoEspeci', marca: marca, modelo:auto_modelo, fechas:fechas, motor:motor, clasific: cla_pieza, nombre:pieza_nom, num_motor:num_motor, chasis:chasis}, function(datos) {
    var piezas= JSON.parse(datos);
